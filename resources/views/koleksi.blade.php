          ...
          @if(count($koleksi) > 0)
              @foreach($koleksi as $item)
                  <div class="card">
                    ...
                    <img src="{{ asset('storage/' . $item->gambar) }}" class="product-image" alt="Product Image"
                        style="width:100%">
                    @endif
                      <h1 class="nama text-white">{{ $item->nama_barang }}</h1>
                      <p class="title text-white">{{ $item->tahun_abad_masa }}</p>
                      <p class="didapat text-black">{{ $item->cara_didapat }}</p>
                      <a href="{{ route('detailkoleksi_landing', ['id' => $item->id]) }}" class="btn btn-primary py-3 px-5 mt-2">Detail</a>
                  </div>
              @endforeach
          @else
              <div class="text-center">Tidak ada hasil pencarian.</div>
          @endif
      </div>
